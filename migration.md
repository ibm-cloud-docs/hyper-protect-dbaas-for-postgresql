...
|*cert_file*|The [certificate authority (CA)](#x2016383){: term} file in the `.pem` format that you downloaded from the **Manage** page on the service dashboard. You can specify the file with a relative or absolute path.|./cert.pem|
...
|Parameter|Description|Example|
|---------|-----------|-------|
|*host_name*|The name of the host server that hosts the target cluster. You can find the host address on the **Manage** page on the service dashboard.|4e.52.37a9.ip4.static.sl-reverse.com|
...
|*ssl_mode*|The SSL mode for encryption.|verify-full|
|*cert_file*|The CA file in the `.pem` format that you downloaded from the **Manage** page on the service dashboard. You can specify the file with a relative or absolute path.|./cert.pem|
